import { isDesktop, touch, showMenu, initNavToggle, initAnimationObserver, sttBtn, scrollToTop, initSpotlightCursor, spotlight, fetchGalleryData } from './utils.js';
...
function initCardModuleObserver() {
  jsModuleObserver = new IntersectionObserver(async function (entries) {
    for (const entry of entries) {
      const card = entry.target;
      if (!entry.isIntersecting || !card.dataset.module || card.dataset.loader === 'true') continue;
      try {
...
function observeGalleryCards(animationObserver) {
  document.querySelectorAll('.gallery__card:not(.observed)').forEach(function (card) {
...
function setupLoaderObserver() {
  lastCardObserver = new IntersectionObserver(function (entries) {
